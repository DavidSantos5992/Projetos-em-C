/* 16- Escreva um programa que leia um n mero?
...
{
cigual++;
//2. Quantos valores iguais a A existem na diagonal 
principal da matriz;
if (i==j)
cigualdiag++;
// 3. Quantos valores iguais a A existem na primeira coluna
da matriz;
if (j==0)
cigualcol++;
}
...
if (tabela[i][j]>a)
{
// 4. Quantos valores maiores do que A existem na 
matriz;
// 5. O valor da soma dos valores maiores do que A  existentes na matriz.
cmaior++;
amaior += tabela[i][j];
...
printf("A soma dos valores maiores que A   %i\n", amaior);?
return 0;
}
